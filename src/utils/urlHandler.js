...
export class URLHandler {
  ...
  static validateIncomingURL(url) {
    try {
      // Handle TrackPlayer notification URLs - these are valid and should redirect to home
      if (url.startsWith('trackplayer://')) {
        console.log('[URLHandler] TrackPlayer notification URL detected:', url);
        return { 
          isValid: true, 
          type: 'trackplayer_redirect', 
          reason: 'TrackPlayer notification - will redirect to home' 
        };
      }
  ...
  static async handleIncomingURL(url, radioController) {
    ...
    try {
      switch (validation.type) {
        case 'trackplayer_redirect':
          // TrackPlayer URLs are handled at the app level now
          console.log('[URLHandler] TrackPlayer URL will be handled by app-level navigation');
          return true;

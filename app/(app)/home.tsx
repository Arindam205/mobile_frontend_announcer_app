...
import {
  ...
  ActivityIndicator,
  TouchableOpacity,
  AppState, // Added for notification handling
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Radio, ChevronRight, Play, Pause, WifiOff, AlertCircle } from 'lucide-react-native';
import { api } from '../../src/api/config';
...
export default function HomeScreen() {
  ...
  // Add app state tracking for notification handling
  ...
  // NOTIFICATION HANDLING: Add app state monitoring
  useEffect(() => {
    const handleAppStateChange = (nextAppState: any) => {
      console.log('[HomeScreen] App state changed from', appStateRef.current, 'to', nextAppState);
      
      if (appStateRef.current.match(/inactive|background/) && nextAppState === 'active') {
        // App has come to foreground - likely from notification click
        console.log('[HomeScreen] App came to foreground, checking for active playback');
        ...
        setTimeout(async () => {
          try {
            ...
            console.log('[HomeScreen] Current playback state:', currentState.state);
            console.log('[HomeScreen] Queue length:', queue.length);
            
            // If there's an active track or playing state, user likely came from notification
            if (queue.length > 0 || currentState.state === State.Playing || currentState.state === State.Paused) {
              console.log('[HomeScreen] Active playback detected, ensuring proper UI state');
              
              // Update the UI to reflect current playback state
              const currentTrack = await TrackPlayer.getActiveTrack();
              if (currentTrack) {
                const trackId = currentTrack.id;
                const channelIdMatch = trackId?.match(/channel-(\d+)-/);
                if (channelIdMatch) {
                  const channelId = parseInt(channelIdMatch[1], 10);
                  setCurrentPlayingChannel(channelId);
                  
                  if (currentState.state === State.Playing) {
                    setIsBuffering(false);
                  }
                  
                  console.log('[HomeScreen] Updated UI for channel:', channelId);
                }
              }
            }
          } catch (error) {
            console.error('[HomeScreen] Error checking playback state:', error);
          }
        }, 300);
      }
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      // Enhanced focus effect: Also check playback state when screen comes into focus
      const checkPlaybackState = async () => {
        try {
          ...
          console.error('[HomeScreen] Error in focus effect:', error);
        }
      };
